...
const BINARYOPERATORS: Set<string> = new Set<string>(["+", "-", "*", "/", "^"])
...
export function evaluateExpression (exp: string): number {
    ...
    let tokens: string[] = parseInfixExpression(exp);
    let valueStack: Stack<number> = new Stack<number>();
    let opStack: Stack<string> = new Stack<string>();

    let implicitMultiplyFlag: number = 0;
    tokens.forEach(token => {
        if(isNumeric(token)) {
            ...
            if(opStack.peek() !== LEFTPARENTHESIS) throw new MismatchParenthesisError();
            opStack.pop(); 
...
function parseInfixExpression(exp: string): string[] {
    let tokens: string[] = [];
    
    
    let prevState: number = STATE.NONE;
    let numParse: string = "";
    for (let i = 0; i < exp.length; i++) {
        let currState: number = defineLiteralState(exp[i], prevState);
        ...
        if (currState === STATE.NUMBER || currState === STATE.DECIMAL || currState === STATE.DECIMALSEPARATOR) {
            numParse += exp[i];
        }
        else {
            if(numParse !== ""){
...
const defineLiteralState = (literal: string, prevState: number) : number => {
    if(isNumeric(literal)) {
        if(prevState === STATE.BLANKAFTERNUMBER
    ...
    if(literal === " ") {
        if(prevState === STATE.DECIMALSEPARATOR)
            throw new InvalidSyntaxError();
        if(prevState === STATE.NUMBER
            || prevState === STATE.DECIMAL)                 return STATE.BLANKAFTERNUMBER;
    ...
    if(BINARYOPERATORS.has(literal)) {
        if (prevState != STATE.NUMBER
            ...
            if (literal === "-"
                && prevState != STATE.BLANKAFTERNUMBER)     return STATE.NUMBER;
...
const evaluatePreceding = (valueStack: Stack<number>, opStack: Stack<string>, token: string) => {
    ...
    while(!opStack.isEmpty()) {
        if (PRECEDENCE[token] <= PRECEDENCE[opStack.peek()]){
            let num2: number = valueStack.pop();
            let num1: number = valueStack.pop();
            let operator: string = opStack.pop();
...
function calculateExpression (num1: number, num2: number, operator: string): number {
    switch (operator) {
        case "+":
            return (num1 + num2);
        case "-":
